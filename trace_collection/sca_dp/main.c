...
/* Control thread: hooks the kernel exit handler and waits until the kernel clears the sync flag. */
void* ctrl_thread(void* arg) {
    ...
    while (*(volatile uint8_t*)sev_step_kernel_sync_addr_p)
        sched_yield();
...
int main(int argc, char **argv) {
    ...
    /* Build the sync flag.
     * The flag encodes:
     *   - The number of code pages to record (shifted by SYNC_FLAG_CODE_PAGE_NUM_SHIFT)
    ...
    flag = (code_page_num << SYNC_FLAG_CODE_PAGE_NUM_SHIFT)
         ...
         | (PMC_WRAPPER << SYNC_FLAG_PMC_WRAPPER_SHIFT)
         | HOOK_SWITCH;
    ...
    write8(sev_step_kernel_sync_addr_p, SYNC_OFFSET_MEM_MAPPED_STACK_SIZE, MEM_MAPPED_QUEUE_SIZE);
    write8(sev_step_kernel_sync_addr_p, SYNC_OFFSET_JIT, JIT_SWITCH);
    write8(sev_step_kernel_sync_addr_p, SYNC_OFFSET_WX_TIMES, 1);
    write64(sev_step_kernel_sync_addr_p, SYNC_OFFSET_PMC_CLF_SW_THRESHOLD, PMC_CLF_SW_THRESHOLD);
